...
def lookup_game_by_id_or_title(appid: int = None, title: str = None) -> dict | None:
    ...
    def fetch_app_details(appid: int) -> dict | None:
        ...
        try:
            ...
            if not data or str(appid) not in data or not data[str(appid)]["success"]:
                log(f"lookup_game_by_id_or_title: No data for appid {appid}", VERBOSITY_WARNING)
                return None
            return data[str(appid)]["data"]
        except Exception as e:
            log(f"lookup_game_by_id_or_title: Exception fetching appid {appid}: {e}", VERBOSITY_ERROR)
            return None
    ...
    def search_app_by_name(name: str) -> int | None:
        ...
        try:
            ...
            log(f"lookup_game_by_id_or_title: Exception searching title '{name}': {e}", VERBOSITY_ERROR)
            return None
    ...
    if appid:
        ...
        if found_appid is None:
            log(f"lookup_game_by_id_or_title: No appid found for title '{title}'", VERBOSITY_WARNING)
            return None
        app_data = fetch_app_details(found_appid)
    else:
        log("lookup_game_by_id_or_title: Neither appid nor title provided.", VERBOSITY_ERROR)
        return None
...
if __name__ == "__main__":
    ...
    if args.strategy == "owned":
        ...
        if not args.id and not args.title:
            log("When using strategy=lookup, you must provide --id or --title.". VERBOSITY_ERROR)
        ...
        if game:
            ...
            log("No matching game found.",VERBOSITY_WARNING)
